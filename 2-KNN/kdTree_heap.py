...
class kdTree_heap(object):
    ...
    def knn_algo(self, item, k=1):
        ...
        heapq.heappush(node_list, (cur_distance, tuple(np.append(node.item, node.label[0]))))
        ...
        while True:
            ...
            cur_distance = heapq.nsmallest(k, node_list)[-1][0]
            # 判断父结点到测试点的距离, 若小于当前最短距离, 则更新当前最短距; 若node_list中结点数不足k, 也把该点信息加入
            if k > len(node_list) or par_distance < cur_distance:
                heapq.heappush(node_list, (par_distance, tuple(np.append(parent.item, parent.label[0]))))
                cur_distance = heapq.nsmallest(k, node_list)[-1][0]
        ...
        label_dict = {}
        node_list = heapq.nsmallest(k, node_list)
        # print(node_list)
        for element in node_list:
            if element[1][-1] in label_dict:
                label_dict[element[1][-1]] += 1
            else:
                label_dict[element[1][-1]] = 1
